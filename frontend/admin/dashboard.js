...
async function loadAllStudents() {
    const results = await Promise.all(
        allClasses.map(c =>
            authFetch(`${API}/students/class/${c.class}`)
                .then(r => r.json())
...
async function loadSummary() {
    ...
    const res = await authFetch(`${API}/dashboard/summary?month=${month}&year=${currentYear}`)
...
async function loadClassProgress() {
    ...
    const results = await Promise.all(
        allClasses.map(c => {
            ...
            return authFetch(`${API}/fees/class/${c.class}/month/${month}/year/${currentYear}${sectionParam}`)
                .then(r => r.json())
...
async function loadPending() {
    const month = MONTHS[currentMonth]
    const res = await authFetch(`${API}/fees/pending/all?month=${month}&year=${currentYear}`)
    const data = await res.json()
...
function renderClassProgress(results) {
    ...
    list.innerHTML = results.map(({ cls, students }) => {
        ...
        return `
        <div class="progress-card" onclick="window.location.href='fees.html?class=${cls.class}&section=${encodeURIComponent(cls.section || '')}&month=${month}&year=${currentYear}'" style="cursor:pointer">
            <div class="progress-top">
                <div class="progress-class">
                    <div class="progress-class-name">Class ${cls.class}${cls.section ? ' — ' + cls.section : ''}</div>
                    <div class="progress-teacher">${esc(cls.teacher_name || 'No teacher')}</div>
                </div>
...
function renderRecent(fees) {
    ...
    list.innerHTML = fees.map(f => {
        const timeAgo = getTimeAgo(f.paid_at || f.created_at)
        return `
        <div class="recent-row">
            <div class="recent-avatar">${f.student_name ? f.student_name[0].toUpperCase() : '?'}</div>
            <div class="recent-info">
                <div class="recent-name">${esc(f.student_name || '—')}</div>
