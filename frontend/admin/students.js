...
function getFeeBadge(studentId) {
    ...
    const feeUrl   = `fees.html?class=${classId}&month=${currentMonth}&year=${currentYear}&student_id=${studentId}`
    const collectUrl = `fee-collect.html?student_id=${studentId}&month=${currentMonth}&year=${currentYear}`

    if (!fee) {
        return `<span class="fee-badge-sm unpaid"
            onclick="event.stopPropagation(); window.location.href='${collectUrl}'">
            Unpaid
        </span>`
    }
    return `<span class="fee-badge-sm ${fee.status}"
        onclick="event.stopPropagation(); window.location.href='${feeUrl}'">
    ...
    list.innerHTML = allStudents.map(s => `
        <div class="student-row ${selectedRolls.has(s.roll_no) ? 'selected' : ''}" id="row-${s.roll_no}">
            ...
            <button class="btn-view ${selectionMode ? 'hidden' : ''}"
                    onclick="event.stopPropagation(); window.location.href='student-detail.html?roll_no=${s.roll_no}&class=${classId}&section=${encodeURIComponent(sectionId)}'">
                View
            </button>
        </div>
    `).join('')
...
async function loadFeeStatus() {
    try {
        const url = `${API}/fees/class/${classId}/month/${currentMonth}/year/${currentYear}${sectionParam}`
        ...
        // non-fatal — fee badges will just show 'Unpaid'
    }
}
...
async function loadClassInfo() {
    ...
    // disambiguate by section if provided
    const cls = classes.find(c =>
        String(c.class) === String(classId) &&
        (sectionId === '' || (c.section || '') === sectionId)
    )
    if (!cls) return

    document.getElementById('classSummary').innerHTML = `
        <div class="cs-class">Class ${cls.class}${cls.section ? ' — ' + cls.section : ''}</div>
        <div class="cs-divider">|</div>
        <div class="cs-teacher">
            👤 ${esc(cls.teacher_name || 'No teacher')}
...
async function loadStudents() {
    const res      = await authFetch(`${API}/students/class/${classId}${sectionParam}`)
