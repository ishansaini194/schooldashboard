...
//   - state at top → useState
//   - fetch functions → custom hooks
...
function renderSummary(data) {
    ...
    document.getElementById('statPending').textContent = data.overdue_count || 0

    // alert banner
    if (data.overdue_count > 0) {
        document.getElementById('alertBanner').classList.remove('hidden')
        document.getElementById('alertText').textContent =
            `${data.overdue_count} partial payment${data.overdue_count > 1 ? 's' : ''} overdue`
    } else {
        document.getElementById('alertBanner').classList.add('hidden')
    }
}

// → ClassProgressCard component
function renderClassProgress(results) {
    ...
    list.innerHTML = results.map(({ cls, students }) => {
        ...
        const paid = students.filter(s => s.has_paid && s.fees?.every(f => f.status === 'paid')).length
        ...
        <div class="progress-card" onclick="window.location.href='fees.html?class=${cls.class}&month=${month}&year=${currentYear}'" style="cursor:pointer">
            ...
            <div class="progress-footer">
                ...
                ${total - paid > 0 ? `<span class="progress-pending">${total - paid} pending</span>` : '<span class="progress-done">✓ All paid</span>'}
            </div>
        </div>`
    }).join('')
}
...
function renderPending(pending) {
    ...
    list.innerHTML = shown.map(s => `
        <div class="pending-row ${s.status}">
            ...
            <button class="btn-mini" onclick="window.location.href='fee-collect.html?student_id=${s.student_id}&month=${MONTHS[currentMonth]}&year=${currentYear}'">
                Pay
            </button>
        </div>
    `).join('')
    ...
    if (pending.length > 8) {
        list.innerHTML += `<div class="pending-more">+${pending.length - 8} more — <a href="fees.html">View all</a></div>`
    }
}

// → RecentPaymentRow component
function renderRecent(fees) {
    ...
    list.innerHTML = fees.map(f => {
        ...
        <div class="recent-row">
            ...
            <button class="btn-mini secondary" onclick="window.location.href='fee-receipt.html?receipt=${f.receipt_no}'">
                Receipt
            </button>
        </div>`
    }).join('')
